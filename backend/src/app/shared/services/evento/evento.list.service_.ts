...
*Autor: Italo Schulz
*Fecha: 30-08-2018
*Modulo: Service Listar Hora(tabla) / Listar Editar
*Información: 
*/

//Importaciones Dependientes del servicio.
...
export class EventoListService {
  ...
  constructor(public http: HttpClient, public perPage?: number) {
  ...
  delete(item: any): Promise<any> {
     ...
     let id = JSON.stringify(item._id);
     let ida=id.replace(/['"]+/g, '');
     return this.http.delete('http://localhost:3789/api/get_evento/'+ida, {headers:headers})
      .toPromise()
      .then(() => null)
