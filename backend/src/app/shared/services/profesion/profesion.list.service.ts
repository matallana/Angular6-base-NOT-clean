...
*Autor: Italo Schulz
*Fecha: 01-2018-2019
*Modulo: Service Listar Profesion (tabla)
*Información: 
*/
...
export class DemoService2 implements DataSource {
  public url: string;
  ...
  page(data: any, page: any): Array<any> {
    ...
    return console.log("PAGE START AND END ===> ", start, end), data.slice(start, end);
  }
  ...
  put(item: any): Promise<any> {
    ...
    return new Promise((resolve) => {
      console.log('Resolve -->', resolve),console.log('ida -->', ida),console.log('item -->', item),
        this.http.put('http://localhost:3789/api/update_ocupation/'+ida, item, {headers:headers})
       .toPromise().then(() => null).catch(this.errorHandler),
       setTimeout(() => resolve(item), 950);
    });
  }
  ...
  delete(item: any): Promise<any> {
     ...
     let id = JSON.stringify(item._id);
     let ida=id.replace(/['"]+/g, '');
     return this.http.delete('http://localhost:3789/api/get_ocupation/'+ida, {headers:headers})
      .toPromise()
      .then(() => null)
