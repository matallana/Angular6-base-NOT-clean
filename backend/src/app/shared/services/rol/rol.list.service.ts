...
export class DemoService4 implements DataSource {
  public url: string;
  ...
  put(item: any): Promise<any> {
    ...
    return new Promise((resolve) => {
      console.log('Resolve -->', resolve),console.log('ida -->', ida),console.log('item -->', item),
        this.http.put('http://localhost:3789/api/get_rol/'+ida, item, {headers:headers})
  ...
  delete(item: any): Promise<any> {
     ...
     let id = JSON.stringify(item._id);
     let ida=id.replace(/['"]+/g, '');
     return this.http.delete('http://localhost:3789/api/get_rol/'+ida, {headers:headers})
      .toPromise()
      .then(() => null)
