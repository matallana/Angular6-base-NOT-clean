...
*observacion: no se ha realizado eliminar ocupation.
*/
'use stric'
...
function updateOcupation(req, res){
    ...
    Ocupation.findByIdAndUpdate(ocupationId, update, { new:true },(err, ocupationUpdate)=>{
        if(err) return res.status(500).send({message: 'Error en la peticion'});
        if(!ocupationUpdate) return res.status(404).send({message:'Error al actualizar la Empresa'});
        return res.status(200).send({empresa: ocupationUpdate});
    });
}
//    
...
function getOcupation(req, res){
    ...
    Ocupation.find(ocupationId,(err,ocupations)=>{
        console.log('ocupationsID', ocupations)
        if(err) return res.status(500).send({message:'Error en la solicitud de ocupations'});
