...
function getUserAll(req, res){
 ...
 function getUsersbyempresa(req, res){
    var empresaId = req.params.id;
    User.find({empresa : empresaId, rol: '5b8eb1ad50e8514f56bd87f7'} ,(err,users)=>{
...
function getDireccionsbyempresa(req, res){
    var empresaId = req.params.id;
    User.find({empresa : empresaId, rol: '5cadfe27dd4ed99d1c2f8379'} ,(err,users)=>{
        if(err) return res.status(500).send({message:'Error en la solicitud'});
        if(!users) return res.status(400).send({message:'La empresa no existe'});
        res.status(200).send(users);
